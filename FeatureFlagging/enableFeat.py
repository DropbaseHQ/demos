...
def enableFeat(state: State, context: Context) -> Context:
    engine = create_engine(DEMO_DATABASE_URL)
    connection = engine.connect()
    response = connection.execute(
        f"UPDATE feature SET is_enabled = true WHERE id = {state.tables.features.id}"
    )
    if response.rowcount > 0:
        context.widgets.featureController.message = f"Successfully enabled feature!"
        context.widgets.featureController.message_type = "success"
    else:
        context.widgets.featureController.message = "Unable to enable feature!"
